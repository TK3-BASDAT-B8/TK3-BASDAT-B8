...
def _ticket_context(request, rows=None, **extra):
    ...
    all_rows = _fetch_ticket_rows(request)
    role = page_role(request)
    ...
    ctx = {
        ...
        'stats': {
            'total': len(all_rows),
            'valid': sum(1 for r in all_rows if _normalize_payment_status(r.get('payment_status')) == 'Paid'),
            'used': sum(1 for r in all_rows if _normalize_payment_status(r.get('payment_status')) == 'Used'),
            'cancelled': sum(1 for r in all_rows if _normalize_payment_status(r.get('payment_status')) == 'Cancelled'),
        },
        'page_role': role,
